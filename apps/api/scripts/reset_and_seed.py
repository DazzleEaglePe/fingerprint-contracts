"""Script para limpiar e insertar datos de prueba completos en la BD."""
...
async def reset_and_seed():
    async with async_session_maker() as session:
        print("🗑️ Limpiando tablas para evitar duplicados...")
        await session.execute(text("TRUNCATE TABLE biometric_verifications CASCADE;"))
        await session.execute(text("TRUNCATE TABLE biometric_templates CASCADE;"))
        await session.execute(text("TRUNCATE TABLE contracts CASCADE;"))
        await session.execute(text("TRUNCATE TABLE owners CASCADE;"))
        await session.execute(text("TRUNCATE TABLE fundos CASCADE;"))
        ...
        await session.commit()
        print("✅ Base de datos reseteada y lista para pruebas intensivas sin duplicados.")
